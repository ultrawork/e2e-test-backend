...
async function globalSetup() {
  ...
  try {
    ...
    // prisma migrate deploy failed (e.g., tables already exist from db push)
    // — ensureJoinTable below will handle it
  }

  // Always ensure the _CategoryToNote join table and FK constraints exist,
  // whether prisma migrate deploy succeeded, failed, or was a no-op
  await ensureJoinTable(DATABASE_URL);
}
